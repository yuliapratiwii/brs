...
<head>
    ...
    <title>@yield('title', 'Agenda Penomoran BRS')</title>
    ...
    <header class="bg-navy text-white">
        <div class="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
            ...
            <div class="flex items-center gap-5">
                <a href="{{ route('brs.index') }}" class="hover:text-brass transition {{ request()->routeIs('brs.*') ? 'text-brass' : 'text-slate-200' }}">Registrasi</a>
                <a href="{{ route('settings.index') }}" class="hover:text-brass transition {{ request()->routeIs('settings.*') ? 'text-brass' : 'text-slate-200' }}">Pengaturan</a>
            </div>
